...
export const Magnetic = ({ children, strength = 0.3, enabled = true, className = '', }) => {
    const elementRef = useRef(null);
    const { state } = useCursorContext();
    const [position, setPosition] = useState({ x: 0, y: 0 });
    const applyMagneticEffect = useCallback((mouseX, mouseY) => {
    ...
    return (_jsx("div", { ref: elementRef, className: className, onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave, style: {
